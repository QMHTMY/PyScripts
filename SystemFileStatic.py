...
def draw(args):
    ...
    tickbar = "┊\n"
    ax.set_xticklabels([f'{tickbar*(i%3)}{numfmt(bins[i])}~{numfmt(bins[i+1])}'
                        for i in range(len(hist)-1)] + [f"{numfmt(bins[len(hist)-1])}~"])
    # set ax
    ...
    # label on top of every bar, uplefted a little
    for i in range(len(hist)):
        ax.text(i - 0.5, hist[i] + files / 400, f"{hist[i]:5}") 
